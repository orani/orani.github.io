...
function piirra(kuvadata,otsakkeet,otsikko,kohde,tyyppi) {
	  ...
	  if (tyyppi=="100") {
		for (x=1;x<kuvadata.length;x++) {	
			...
			for (i=0;i<kuvadata[0].length;i++) 	{ kuvadata[x][i]=(kuvadata[x][i]/vuosi_nolla)*100;	} 		}
		prosenttia=false;
	  }
	   if (tyyppi=="0") {
		for (x=1;x<kuvadata.length;x++) {		
	...
	if (kuvadata.length==5) {
	  ...
	  if (tyyppi=="0" || tyyppi=="100" || kohde=="huoltosuhdekuva") {  var chart =  new google.visualization.LineChart(document.getElementById(kohde)); 	 } 
